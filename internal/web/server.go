...
func (s *Server) routes() {
	...
	s.mux.Handle("POST /firewall/flowtables/{id}/delete", s.requireAuth(s.handleFlowtableDelete))
	...
	s.mux.Handle("POST /harden/ban", s.requireAuth(s.handleHardenBan))
	s.mux.Handle("POST /harden/unban", s.requireAuth(s.handleHardenUnban))
	...
	s.mux.Handle("POST /settings/backup/auto", s.requireAuth(s.handleBackupAuto))
	s.mux.Handle("GET /settings/backup/file/{name}", s.requireAuth(s.handleBackupFileDownload))
